...
def second_and_third(the_list):
    changed_list = [[the_list[1]],[the_list[2]]]
    print (changed_list)
    changed_list.sort(reverse = True)
    print(changed_list)
    return changed_list
...
def concatenate_sentences(sentence1, sentence2):
    if sentence1[0].isupper() and sentence1.endswith(('.', '?', '!')):
        new_sentence1 = sentence1.lstrip()
        new_sentence2 = sentence2.lstrip()
        print(new_sentence1 + ' ' + new_sentence2)
    else:
